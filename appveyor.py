...
class AppveyorApi:
    ...
    def getHistory(self):
        self.historyurl=urljoin(self.apiurl,"projects",self.account_name,self.project_name,"history?recordsNumber=100"+self.branch)
    ...
    def getDlUrl(self,keyword="",no_keyword="/",filetype="7z",no_pull=False):
        for version in self.getHistory():
            ...
            self.buildurl = urljoin(self.apiurl,"projects",self.account_name,self.project_name,"build",self.version)
            ...
            self.artifactsurl=urljoin(self.apiurl,"buildjobs",self.jobid,"artifacts")
            ...
            for fileinfo in self.artifactsjson:
                ...
                if keyword in filename and no_keyword not in filename and filename[-len(filetype):]==filetype:
                    ...
                    self.dlurl=urljoin(self.apiurl,"buildjobs",self.jobid,"artifacts",self.filename)
                    return self.dlurl
...
class GithubApi:
    def __init__(self,account_name,project_name):
        ...
        self.account_name = account_name
        self.project_name = project_name
        self.releasesurl = urljoin(self.apiurl,self.account_name,self.project_name,"releases")
...
class GithubSpider:
    def __init__(self,account_name,project_name):
        ...
        self.releasesurl = self.domain+"/"+self.account_name+"/"+self.project_name+"/releases"
